...
// Registrar el ID de partida recibido
...
if ($id_partida > 0) {
    ...
    // Ejecutar la consulta para verificar el estado
    if ($stmt_check_estado->execute()) {
        $stmt_check_estado->bind_result($estado);
        $stmt_check_estado->fetch();
        $stmt_check_estado->close();
        
        // Registrar el estado de la partida
        ...
        echo "Error al consultar el estado de la partida: " . $stmt_check_estado->error;
    }
} else {
