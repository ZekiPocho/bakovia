...
if ($result) {
    if ($estado === 'finalizado') {
        ...
        $query_puntajes = "SELECT puntaje_usuario1, puntaje_usuario2 FROM partida WHERE id_partida = $id_partida";
        $result_puntajes = mysqli_query($conn, $query_puntajes);
        
        if ($result_puntajes) {
            $row_puntajes = mysqli_fetch_assoc($result_puntajes);
            $puntaje_jugador1 = $row_puntajes['puntaje_usuario1'] ?? 0; // Asumiendo que tienes estas columnas
            $puntaje_jugador2 = $row_puntajes['puntaje_usuario2'] ?? 0;
