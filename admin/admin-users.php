...
// Eliminar usuario y sus publicaciones y comentarios
...
<body>
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                <a href="admin-users.php?delete_user=<?php echo $user['id_usuario']; ?>" onclick="return confirm('¿Estás seguro de eliminar este usuario y todas sus publicaciones y comentarios?')">Eliminar</a>
            </td>
        </tr>
        <tr>
            <td colspan="4">
                <h2>Publicaciones de <?php echo $user['nombre']; ?></h2>
                <?php
                // Obtener publicaciones del usuario
                $id_usuario = $user['id_usuario'];
                $post_query = "SELECT * FROM publicaciones WHERE id_usuario = $id_usuario";
                $post_result = mysqli_query($conn, $post_query);

                while ($post = mysqli_fetch_assoc($post_result)): ?>
                    <div class="post">
                        <strong>ID Publicación:</strong> <?php echo $post['id_publicacion']; ?><br>
                        <strong>Título:</strong> <?php echo $post['titulo']; ?><br>
                        <strong>Contenido:</strong> <?php echo substr($post['contenido'], 0, 100); ?>...
                        <a href="edit-post.php?id=<?php echo $post['id_publicacion']; ?>">Editar</a> |
                        <a href="admin-users.php?delete_post=<?php echo $post['id_publicacion']; ?>" onclick="return confirm('¿Estás seguro de eliminar esta publicación y sus comentarios?')">Eliminar</a>
                    </div>
                    <h2>Comentarios</h2>
                    <?php
