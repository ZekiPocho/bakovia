...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Preparar la consulta para eliminar la partida
    $sqlDeletePartida = "DELETE FROM partida WHERE id_partida = ?";
    $sqlUpdateMade = "UPDATE usuarios SET made = 0 WHERE nombre_usuario IN (SELECT nombre_usuario1 FROM partida WHERE id_partida = ?) OR nombre_usuario IN (SELECT nombre_usuario2 FROM partida WHERE id_partida = ?)";
