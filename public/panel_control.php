...
// Obtener el id_partida de alguna fuente, como un parámetro GET en la URL
$id_partida = $_GET['id_partida'];  // Asegúrate de hacer una validación de seguridad aquí

// Consulta para obtener la partida en función de su ID
$sql = "SELECT p.id_partida, p.id_juego, p.puntos, 
            p.nombre_usuario1, 
            p.nombre_usuario2, 
            p.puntaje_usuario1, p.puntaje_usuario2,
            p.hora_inicio, p.id_mesa
        FROM partida p
        WHERE p.id_partida = ?"; // Usamos una consulta preparada

// Preparar y ejecutar la consulta
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $id_partida);  // Vincular el parámetro
$stmt->execute();
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $ahora = time();
        $tiempo_transcurrido = gmdate("H:i:s", $ahora - $hora_inicio);
?>
    <!-- Aquí empieza el HTML para mostrar el panel de control -->
    <div class="control-panel mb-2 text-center">
        <div class="row align-items-center">
            <!-- Columna 1: Jugador 1 -->
            <div class="col-3">
                <span><?php echo $row['nombre_usuario1']; ?></span>
                <div>Puntaje: <?php echo $row['puntaje_usuario1']; ?></div>
            </div>
            
            <!-- Columna 2: Información del juego -->
            <div class="col-6">
                <h1><?php echo $row['id_juego']; ?> - MESA <?php echo $row['id_mesa']; ?></h1>
                <h1><?php echo $row['puntos']; ?> Pts.</h1>
                <div class="timer">
                    <i class="lni lni-hourglass"></i> Tiempo transcurrido: <?php echo $tiempo_transcurrido; ?>
                </div>
            </div>
            ...
            <div class="col-3">
                ...
                <div>Puntaje: <?php echo $row['puntaje_usuario2']; ?></div>
            </div>
        </div>
    </div>
<?php
